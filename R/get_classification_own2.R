...
inchi_key<-as.character("BRMWTNUJHUMWMS-LURJTMIESA-N")

get_classification_own <- function(inchi_key, conn=NULL)
{
    if (is.null(conn)!=TRUE)  {
      qry<-dbSendQuery(conn, "SELECT InChiKey,Classification FROM classyfire WHERE InChiKey=?")
      dbBind(qry,list(inchi_key))
      key<-dbFetch(qry)
    }
    
    if (identical(key$InChikey, character(0))!=TRUE) {
      ...
      if (response$status_code == 200) {
        ...
        if (length(json_res$external_descriptors) > 0) {
          object@external_descriptors <- classyfireR:::parse_external_desc(json_res)
        } else{
        ...
        if (is.null(conn)!=TRUE)  {
          qry2<-dbSendQuery(conn,"INSERT INTO classyfire (InChiKey,InChi,Classification) VALUES(?,?,?)")
          dbBind(qry2, list(inchi_key,"NULL",rawToChar(serialize(object,connection=NULL, ascii=TRUE))))
          dbClearResult(qry2)
        }
      }
      return(object)
...
get_classification_own("BRMWTNUJHUMWMS-LURJTMIESA-N")
